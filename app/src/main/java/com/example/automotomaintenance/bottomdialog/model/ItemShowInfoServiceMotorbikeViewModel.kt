...
@HiltViewModel
class ItemShowInfoServiceMotorbikeViewModel @Inject constructor() : ViewModel() {

    @Inject
    lateinit var fifeBaseRepository: FifeBaseRepository
    val infoService = MutableLiveData<List<InformationDB>>()

    fun getItemInfoServiceMotorbike(idService: String) {
        viewModelScope.launch(Dispatchers.IO) {
            val serviceInfo = fifeBaseRepository.loadOneInfoServiceMotorbike(idService)
    ...
    fun updateInfoServiceMotorbike(km: String, idService: String) {
        fifeBaseRepository.upDateInfoServiceMotorbike(km, idService)
    }
}